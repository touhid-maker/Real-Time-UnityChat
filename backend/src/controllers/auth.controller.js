...
export const signup = async (req, res) => {
    ...
    try {
        ...
        if (password.length < 8) {
            return res.this.status(400).json({ message: "Password must be at least 8 characters" });
        }

        // check email if valid: regex
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(email)) {
            return res.this.status(400).json({ message: "Invalid email format" });
        }
